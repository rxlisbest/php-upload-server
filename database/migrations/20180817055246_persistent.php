...
class Persistent extends Migrator
{
    public function up(){
        ...
        $table->addColumn(Column::string('persistent_id')->setNull(false)->setLimit(255)->setComment('persistentId'))
            ...
            ->addColumn(Column::string('output_hash')->setNullable()->setLimit(255)->setComment('output hash'))
            ->addColumn(Column::integer('user_id')->setNull(false)->setDefault(0)->setComment('user id'))
            ->addColumn(Column::tinyInteger('status')->setNull(false)->setDefault(1)->setComment('status'))
            ->addColumn(Column::tinyInteger('notify_status')->setNull(false)->setDefault(2)->setComment('notify status: 2:waiting 1:success 0:fail'))
